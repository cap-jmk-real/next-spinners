...
Server Components cannot import these modules directly. For App Router, keep spinners in a small client leaf (see [examples README](https://github.com/cap-jmk-real/next-spinner-kit/blob/main/examples/README.md)).
...
| `style`      | `CSSProperties`   | `n/a`       | Merged **after** built-in styles; `style.color` overrides `color`.   |
| `label`      | `string`          | `"Loading"` | `aria-label` on the `status` live region.                            |
| `durationMs` | `number`          | `n/a`       | Length of one full loop; interval ≈ `durationMs /` number of frames. |
| `intervalMs` | `number`          | `n/a`       | Ms between frames; overrides built-in timing and `durationMs`.       |
| `...`        | `HTMLSpanElement` | `n/a`       | Normal `<span>` attributes (`className`, `id`, `data-*`, etc.).      |
